...
exports.registerUser = (req, res) => {
  ...
  const result = registerSchema.validate({ name, email, password, department});
  if(!result.error) {
  ...
  if(!result.error) {
    ...
    User.findOne({ email }).then((user) => {
      ...
      bcrypt.compare(password, user.password).then((isMatch) => {
        ...
        const sessUser = { id: user.id, name: user.name, email: user.email, department: user.department };
